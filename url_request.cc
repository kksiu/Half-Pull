...
URLRequest::URLRequest(const GURL& url,
  ...
  //PROJ: add request for URL
  //std::cout << "Construct URL Request : " + url.spec() << std::endl;
...
/*
void CreateUDPAddr(std::string ip_str, int port, IPEndPoint* address) {
...
int WriteSocket(UDPClientSocket* socket, std::string msg) {
	...
	scoped_refptr<DrainableIOBuffer> buffer(new DrainableIOBuffer(io_buffer, length));
	int bytes_sent = 0;
	while (buffer->BytesRemaining()) {
		int rv = socket->Write(buffer, buffer->BytesRemaining(), callback.callback());
...
void URLRequest::StartJob(URLRequestJob* job) {
  ...
  //PROJ
  //std::cout << "URLRequest::StartJob " << url().spec() << std::endl;

  std::string u = url().spec();
  
  if (u.find("http://localhost/cache/") == 0) {
  	std::cout << "Send pulling request " << std::endl;

	int sockfd;
	struct sockaddr_in servaddr;

	std::string prefix = std::string("http://localhost/cache/");
	std::string instruction = u.substr(prefix.length());

	printf("instruction = %s\n", instruction.c_str());

	std::string result = "PULL " + instruction;
	for (size_t i = 0; i < result.length(); i ++) {
		if (result[i] == ',') result[i] = ' ';
	}

	const char* sendline = result.c_str();
	...
	/*
	const int serverPort = 5300;
	IPEndPoint server_address;

	CreateUDPAddr("127.0.0.1", serverPort, &server_address);
	UDPClientSocket client(DatagramSocket::DEFAULT_BIND, RandIntCallback(), NULL, NetLog::Source());
	
	int rv = client.Connect(server_address);
	
	std::cout << "UDP connection : " << rv << std::endl;

	std::string message = std::string("Pull 60 http://www.noi.cn/");

	rv = WriteSocket(&client, message);
	//client.reset();
	
  	std::cout << "Send caching result finished. " << std::endl;
	*/
  }
